...
% ---- Line spacing: 1.5 per thesis guidelines ----
\usepackage{setspace}
\onehalfspacing

% ---- Paragraph gap (guidelines specify line spacing, not parskip) ----


% ---- Extra breathing room around section headings ----
\usepackage{titlesec}
\titlespacing*{\section}{0pt}{2.4ex plus 0.5ex minus 0.2ex}{1.2ex plus 0.2ex}
\titlespacing*{\subsection}{0pt}{1.8ex plus 0.4ex minus 0.2ex}{0.9ex plus 0.1ex}
\titlespacing*{\subsubsection}{0pt}{1.4ex plus 0.3ex}{0.7ex}

% ---- Extra space around floats ----
...
% ---- Verbatim reproduction of prompt files in the appendix ----
\usepackage{listings}
\lstdefinestyle{promptfile}{
...
\begin{document}
	...
	\begin{tabular}{@{}p{0.82\textwidth}r@{}}
		Appendix~A \enspace Custom Claude Code skills (\texttt{discussion} and \texttt{frankGPT}) & \pageref{app:skills} \\[6pt]
		Appendix~B \enspace OpenAlex keyword query & \pageref{app:query} \\
	\end{tabular}
	...
	\begin{longtable}{@{}l p{0.75\textwidth}@{}}
		BBa & BioBrick registry part identifier (naming prefix) \\
	...
	\begin{longtable}{@{}l p{0.72\textwidth}@{}}
		$c$ & a city; the unit of analysis \\
		$P_c,\ Q_c$ & the sets of city $c$'s papers and projects \\
		$\mathbf{e}_i$ & embedding vector of document $i$, with $\mathbf{e}_i \in \mathbb{R}^{768}$ \\
		$\bar{\mathbf{p}}_c,\ \bar{\mathbf{q}}_c$ & paper and project centroids (mean embeddings) of city $c$ \\
		$\bar{\delta}$ & mean project-level alignment (own-city minus other-city similarity) \\
		$O_c$ & Jaccard topic overlap for city $c$ \\
		$N_{\text{proj},c},\ N_{\text{paper},c}$ & project and paper counts in city $c$ \\
		$\gamma_k$ & fixed effect for country $k$ \\
		$\mathbb{1}[\cdot]$ & indicator function (1 if the condition holds, 0 otherwise) \\
		$Y_{c,k,t},\ X_{c,k,t}$ & downstream and upstream shares of cluster $k$ in city $c$, period $t$ \\
		$\alpha_c$ & city fixed effect \\
		$\delta_{k,t}$ & topic-by-period fixed effect \\
		$\beta,\ \hat{\beta}$ & difference-in-differences coefficient and its estimate \\
		$r_M$ & Mantel correlation coefficient \\
		$\hat{\Delta}$ & permutation excess: observed mean overlap minus the null mean \\
		$B$ & number of permutation (or bootstrap) replicates \\
	...
	AI tools were used over the course of this project. Claude Code, Anthropic's agentic coding assistant, was the primary generative tool used; it ran literature searches, processed and cleaned data, generated the language embeddings the analysis rests on, wrote and debugged code, reasoned about methodology, reviewed and constructed drafts, and collated notes. The full project's codebase lives in a public GitHub repository at
	\url{https://github.com/Zer0Juice/synbiomap}. Commits were made by the agent at regular intervals throughout development. The repository's history is a complete audit trail. Every script, notebook, and draft has a timestamped record of what was added, what changed, and why. Anyone who wants to inspect how this project was built, or replicate any part of it, can read it there.
	
	I have chosen to document this use in full rather than minimize it. There is a stigma around AI in academic work, and while much of it is valid and I do have serious concerns over the growing role of generative AI in society, academics are currently incentivized to avoid publicly discussing use of AI. The solution to this period of change lies in openness, disclosure, and education instead of concealment and shame. I believe this project is much deeper and wider in scope than it would have been without the use of AI agents, which were extraordinarily useful in building and maintaining complex bodies of code. The web crawling scripts required to collect the data this analyis is built on would not have been tenable for me to build and run without use of these tools. Natural language document embeddings form a core part of the calculations used throughout this paper.
	
	
	To this project the most valuable and experimental use of claude code was simulating debates or feedback. This was done through a series of extensively documented custom prompts (or claude "skills") . These tools are simply markdown files: plain-text documents that set the project's goals, scope, and voice, and that steered the agents toward my intent. Two of them drove the reasoning work directly. A \texttt{discussion} command staged a debate among several named, real researchers, which pushed the model to draw on those scholars' actual bodies of literature rather than a generic average of the field. A \texttt{frankGPT} command cast the model as an economic-geography advisor in the mold of Frank Neffke, to pressure-test the framing and the empirical strategy. These files are reproduced in the appendix. They are the clearest picture of where the direction came from, worth reading alongside the results, and offer some level of informative entertainment. Of course, all prompt files are available via the github repo linked above.
	...
	For most of human history, the natural world has seemed like a fixed backdrop to our existence, a force as inevitable as it is mysterious, but this old model no longer checks out. As our technological abilities and knowledge of ecology have deepened, it has become very clear that human processes are shaping the biosphere at a momentous scale; we've reached a stage where human activity is the dominant driver of change in Earth's natural systems, so much so that scientists are proposing we have entered a new geological epoch: the Anthropocene \citep{crutzen2006anthropocene}. Whether we set out to or not, we are terraforming our own planet, and the current trajectory does not bode well. Rapidly shifting climate cycles, ocean acidification, collapsing habitats, a thinning web of life; this environment is no longer simply a backdrop we live in front of. Our world is a complex adaptive system we reside inside, molded by our actions whether intentional or otherwise.
	
	There is, however, a tough shard of hope buried in that pile of dread and alarm. The scale of the damage can also be taken as a measure of our power and ability. A species that can warm a planet by accident, with no coordination and no shared intent, has proven something about its reach. If our collective action can disrupt the natural world this much, then our collective action might move it back. This is the thesis behind this research: the power to reshape the biosphere is in our hands. We simply must direct this capability down the right pathways.
	
	We have always tried to understand the living world by taking it apart. For most of its history, biology was a descriptive science, breaking life into organs, then cells, then the genes beneath them. From those pieces, over time, an account of the rules that hold life forms together was developed. Biology had learned to read the language of life, yet still we lacked the ability to write it. That too is changing.
	Once the rules were understood well enough, biologists began to compose with them. A genetic sequence can be treated as a function written in the programming language of life, standardized, and snapped together with others into genetic circuits \citep{wayIntegratingBiologicalRedesign2014}. Synthetic biology is descriptive biology picking up the pen. 
	Of course, manipulating natural systems without fully understanding them is not without its risks. The same command over living systems that lets us read and rewrite a cell can be put to a range of uses. Cells can be engineered to make fuels without a drill, or to pull carbon back out of the air. They can be used as weapons, or as food.  Carbon capture by precision fermentation is an exciting example highlighted throughout this project, where engineered microbes consume carbon dioxide and turn it into something useful. This is not a thought experiment; the technology has been built and implemented at industrial scale. Synthetic biology is one of the most potentially useful tools in our kit for intentionally saving the biosphere.
	
	How that tool gets used is not decided in the lab alone. Synthetic biology is becoming a deeply influential industry, with associated opportunities, risks, and the inevitable regulations that thus emerge. Governments fund research, either directly or through academic institutions, writing rules that permit or forbid various technologies and shaping the markets that receive new innovation. Those choices carry real weight, impacting which problems get worked on and the level of time and money it takes to bring a product to market. A field with this much potential for transformation is also a place where public policy has vital leverage. The decisions made now while the industry is still young and its norms are still forming will shape what it eventually grows into.
	
	Making proper decisions about an emerging field depends on being able to see it clearly. After all, policymakers cannot fund what they cannot find. Understanding how a young field is growing and how its ideas move from one place to another is the practical case for studying innovation in the first place; If we want public investment in synthetic biology to have a meaningful impact, we need some way to trace the field's shape as it forms, rather than reading it off after the fact from a list of finished products. That means measuring research, the raw material of innovation, at a scale where conclusions can be drawn and informed decisions made.
	
	Synthetic biology is unusually difficult to measure and observe clearly from its exterior due to its interdisciplinarity. It grew out of molecular biology, and their vocabularies overlap to a significant degree; a researcher studying the metabolic pathways of \textit{E.\ coli} may have no intention of engineering them, yet would use the same words as a synthetic biologist who does. Oldham and colleagues confronted exactly this problem when building systematic academic and patent maps of the field \citep{oldhamSyntheticBiologyMapping2018}. Synthetic biology can be readily identified from descriptive molecular biology, but it shares so much surface vocabulary with the broader literature that any keyword query pulls in a large number of papers and patents unrelated to engineering biology.
	
	Keyword drift is another source of the problem. Synthetic biology names its new methods and organisms as it discovers them, and today's next big thing may be last year's unnamed result in different terms. For instance, CRISPR, perhaps the most transformative technology for genetics in recent history, was a known if obscure acronym long before it became synonymous with gene editing. If we were to run a keyword query built in 2010, it would miss the root of nearly all the modern genome-editing literature. The same problem applies to specific organisms and parts; cyanobacteria appear in the literature under many genus names and common names depending on the decade and the subfield. Any boundaries for synthetic biology calibrated in 2005 may have drifted by 2015. Drawing a clean and stable line around the field, and retrieving the right corpus of documents, is a genuine technical and conceptual problem that we grapple with throughout the data chapter.
	
	Synthetic biology also comes with a data source that most young fields lack, at the place where the origins of this inquiry begin. The International Genetically Engineered Machine competition, iGEM, is a worldwide student contest in the field, somewhere between a science fair and a startup accelerator. Teams of students, usually hosted by a university, spend most of a year designing and building an original project from a standardized toolkit of synthetic dna fragments. Every team starts from the same set of interchangeable genetic parts and the same basic rules, but they craft a widely diverse range of projects. These are richly documented in a standardized, open source format on designated documentation websites called "wikis" \citep{santoliniIGEMModelSystem2023}. For many, this is their introduction to the field and a launching point into a career in academia or industry. By collecting thousands of these projects, we build a detailed and open record of a field actively composing itself.
	
	To empirically compare a student project against an academic paper or a patent, we need a way to relate documents across fields. We use a natural language model, SPECTER2 from the Allen Institute for AI, to process the titles and abstracts of a documents into vectors that encodes hidden layers of meaning \citep{singhSciRepEvalMultiFormat2023}. When we represent these vectors as points in a high-dimensional space, documents about similar things land near one another, whatever words they happen to use, so the distance between two vectors becomes a measure of how related two pieces of work are. SPECTER2 was trained to predict if papers cite one another, and we use semantic distance as a proxy for the kind of relatedness a citation would mark. It is the core beneath every comparison in this thesis, and the reason we can compare patents, projects, and papers so readily.
	
	The question this we ask is simple. Do cities whose students research certain areas in synthetic biology also produce academic papers, and patents in related areas? Is iGEM research embedded in the topical landscape of a city's research? We arrive at a qualified yes. Measured carefully, a city's student projects and its academic papers tend to fall in the same specific topics. Patents line up topically with papers, and papers line up with projects, though patents and projects remain further in semantic space. We approach this as relatedness and co-location, not interpreting anything as proof of causation.
	
	Prior work on innovation geography measures relatedness using labor flows, product similarity, and co-patent networks, but those methods are designed for large, mature industries and require data on worker mobility or product co-export that simply do not exist for a student-academia-industry pipeline in an emerging field
	\citep{neffkeSkillRelatednessFirm2013, hidalgoProductSpaceConditions2007}. Prior
	work on iGEM has studied team collaboration and career outcomes, but has not connected projects with other products of synbio research. We close this gap by fine-tuning a model on links between the artifact types, building a shared embedding space on a brand-new dat set, and testing the result against an independent validation from physical genetic data the model never touched.
	
	To refine the scope, we focus on carbon capture technology, identifying clusters in the semantic space and exploring them more in-depth. This is one of the most potentially transformative technologies Synthetic Biology could help to build and scale. Cyanobacteria, the blue-green algae, are among the oldest photosynthesizers on the planet, and they earn their place here twice over. They are a favorite chassis for engineered carbon capture, cells that eat sunlight and carbon dioxide and can be tuned to generate fuels or other useful producs in turn. They are also a topic that unites iGEM students, academic labs, and patent-filing firms in the same cities. Following this thread through the data allows us to zoom in on a particular case and see the industry from a different perspective. The core focus of this thesis is about relatedness across a field, but algae are where we watch it happen.
	
	Underneath the embeddings and regressions, the data tells a simple story. Tools for synthetic biology are open. Students learning to use them are doing so, alongside local labs and firms working on the same problems. Knowledge is locally shared across sectors of the industry. Where those students cluster is a signal of where a field is putting down roots. If that signal is real, then funding student science and the open research that sustains it is one of the surest early investments a government can make in an emergent field.
	
	\begin{figure}[htbp]
		\centering
		\includegraphics[width=0.92\linewidth]{pipeline_concept.png}
		\caption{Conceptual model of the three-artifact innovation ladder. Student iGEM projects introduce early-stage biological designs at time $t$; academic papers develop and disseminate those ideas roughly three years later. patents mark downstream commercial translation at a further lag. All three activities co-occur within a shared urban institutional environment whose role is the central object of study. Arrows label the proposed mechanism; the sequence is the hypothesis this thesis tests.}
	...
	\subsection{Innovation, research, and the role of the state in the Bioeconomy}
	
	Innovation policy depends on the simple concept that money spent on research today pays in the form of capability tomorrow. This idea is not new; Vannevar Bush made the canonical case for it in 1945, arguing that basic science funded by the public and pursued without a fixed application in mind is the seedbed from which practical advances eventually grow \citep{bushScienceEndlessFrontier1945}. This framing has shaped decades of science funding, with the expectation that with support for research, breakthroughs and applications are soon to follow. While this model isn't wrong, it does happen to be incomplete; it treats the flow from discovery to application as an automatic pipeline, freely running on its own once the money enters as input and outputting influential research.
	
	The reality is messier. The pipeline model, with research at one end and products at the other, misses how much the two shape each other, and how much the public sector impacts the private. Governments act both as funders and active participants in innovation. The public sector plays the role of early customers and collaborators that help new technologies cross from lab to market \citep{doblingerGovernmentsPartners2019}. Whether a young technology survives the gap between a working prototype and a viable product often depends on public support. Innovation studies has spent decades documenting how much of this is collaborative and institutional, depending on the surrounding environment rather than on lone innovative genius.
	
	Innovation is built based on the environment around a discovery, and the public sector plays a vital role in creating it. They build the conditions in which research can emerge into capability by funding education and shared infrastructure. Standards must be set that allow for healthy competition, while governments keep an eye on potentially transformative fields so they can act as early partner rather than late regulator. In synthetic biology, where products impact public health, food, energy, and the environment, the state will be vital in determining where new capabilities emerge. Acting as an early partner requires understanding the field, and emerging fields are the hardest to truly know. A government that wants to support synthetic biology has to know where it is happening, who is doing it, and what counts as part of it. However, none of these factors is obvious for a field where boundaries are still forming. Usual instruments will lag behind, with industry classifications and funding categories built for established sectors missing the true innovation that slips under the radar. Technology that does not fit existing boxes is easy to undercount or miss, but the communication gap runs both ways. Researchers struggle to convey the shape of a moving field to the people who fund it, and funders struggle to conceptualize what they are asked to support.
	
	The difficulty is worth the trouble, because new fields are where the largest gains in productivity tend to emerge. New technologies open the most room for growth precisely when they are young and unformed, before their methods settle and their returns level off. This is the frustrating shape of the problem for policy: the fields with the most to gain from good early decisions are the ones we can see least clearly. A synthetic biology that is easy to measure will be a mature synthetic biology, with its trajectory mostly set. If public investment is going to shape the field, it has to act while the picture is still blurry. This highlights the potential importance in any method that can bring the emerging picture of such a field into slightly better focus.
	
	If research is the raw material of innovation, where does it come from and how does it accumulate? It's not random, and it's not evenly distributed; over time, it's natural for certain places to develop an advantage over others. A place, a firm, or a field is most likely to move into activities related to what it already does, because the skills, and knowledge carry over easily \citep{neffkeSkillRelatednessFirm2013a}. Economic Geography has helped in exploring the differences between regions' innovative potential: why different areas move to specialize in different innovative technologies. As cities develop their capabilities in new industries and products, they tend to specialize in certain technologies over others. This could be due to geography (for instance, a city near a body of water will be better equipped to engage in shipping than a landlocked peer located in a desert), demographics (areas with younger populations have a greater advantage in industries requiring manual labor), or a wide range of other factors. As cities specialize in a technology, they continue developing their comparative advantage in this and related areas, branching out into a range of similar innovative technologies \citep{hidalgoPrincipleRelatedness2018} . In the case of SynBio, this would mean that a firm developing capabilities in, for example, scaling industrial fermentation of ethanol would mean enhancing a city's potential to develop capabilties in production of other biofuels.
	
	
	\subsection{A field worth watching}
	
	Synthetic biology is a field where these cycles of innovation, these economic theories around the formation of new industries and technological shocks play out in fast motion. Two cost curves drive it. DNA sequencing has fallen in price faster than computing's own famous Moore's Law decline: the human genome cost roughly three billion dollars to sequence in 2003 and a few hundred dollars today, a drop of seven orders of magnitude in just two decades. The cost of DNA synthesis, writing a custom genetic sequence from scratch, has followed, turning what was once a prohibitively expensive operation into a routine service that any well-equipped lab can order online. As sequencing and synthesis get cheaper, the barrier to designing and testing new biological systems falls, and more researchers enter the field with more hypotheses to try. The result is a field whose rate of change compounds on itself: each year's discoveries lower the cost of the next year's experiments, and the methods change faster than any single literature search can follow. What Drew Endy described in 2005 as the ``foundations'' of a new engineering discipline \citep{endyFoundationsEngineeringBiology2005}, design-build-test cycles using standard interchangeable parts, has since been built out into an industry with commercial gene synthesis companies, automated laboratory platforms, and machine-learning tools that design proteins from scratch. The field is still developing, making it both unusually hard to study and unusually important to track.
	
	The stakes match the speed. The same capability that lets a lab engineer a microbe to capture carbon or produce a medicine could, in other hands, be turned toward harm. These tools have the potential to create incredibly dangerous and destructive bioweapons, or to alter our natural world in fundamental and irreversable ways. While this paper tends towards an optimistic view of this developing industry, the dual-use worry is very real and widely discussed across the field \citep{delisiRoleSyntheticBiology2020}. This is a technology with a genuinely enormous potential to help, deeply real potential to harm, and a trajectory that is still evolving. A field like that should not be left unstudied. Understanding where it is going, and steering it while steering is still possible, is part of our responsibility to engage with technology in a safe and productive manner.
	
	How, then, do we watch and measure a field like this? We read and analyze its outputs. Patents are an obvious place to start when studying new technology; they are, after all, filed because someone believed an idea had enough commercial value to be worth protecting. Patents have long served as a window on the rate and direction of innovation, and a large literature uses them to track how science turns into technology, including the growing extent to which patents cite scientific papers
	\citep{narinIncreasingLinkage1997, marxFuegiRelianceScience2020}. A patent tells us that an idea reached the point of expected commercial use, where it was filed, and roughly when. For a field with industrial impact, this is a signal worth collecting.
	
	The academic paper is the second output type, and it measures innovation from a different perspective. While a patent marks expected commercial value, a paper marks a bid for scientific impact; its authors are claiming a contribution to what the field knows, and the paper's reception measures whether the field agreed. The two signals often diverge. A breakthrough that reshapes a science can have little commercial use, and a lucrative product can rest on an idea that means little to researchers. That divergence is useful. Patents and papers are partial reflections of the same underlying activity, and reading them together gives a fuller picture of a place's capabilities than either does alone.
	
	\subsection{iGEM Projects}
	
	There is another artifact type that makes synthetic biology unusually tractable to study: the student iGEM project. Each team's project is incredibly well documented in a standardized fashion, with much more fine-grained data available than comes with the average publication. They document problems, an approach, methods, experiments, models, results, collaborations, and contributions to the iGEM Registry, tagged with the team's institution and year. The competition runs a public registry of these wikis and of the genetic parts teams create, so the entire archive of iGEM history is accessible and consistently structured. The competition has produced thousands of teams across more than 40 countries, with every team record including location, year and generally a usable abstract. There is no other resource in any scientific field that provides this kind of systematic, open, annual geotagged record of student research at scale
	\citep{santoliniIGEMModelSystem2023}.
	
	A student project carries yet another different kind of signal from a patent or a paper. It records what a group of undergraduates, guided by local mentors, chose to work on with one year and a shared toolkit, without the same level of commercial stake that shapes a patent and reputational pressure that shapes a peer-reviewed article. Both of those stakes constrain what gets filed or published; a student project is shaped mainly by the mentors' research interests, the students' curiosity, and the immense pressure of the competition structure itself, which compresses the timeline of research into a standardized timeline. This shows a different filter into what local research environment cares about, what ideas might be floating around in the air. Where a city's students point their projects is a third view of the city's technical orientation, and the one closest to the beginning of a research career, before the selection pressures of publication and commercialization have started to work their magic.
	
	\subsection{BioBricks and the standardized parts registry}
	
	iGEM's shared toolkit is a public registry of standardized DNA sequences called BioBricks. A BioBrick is a small piece of genetic function with a documented job, like a promoter that switches a downstream gene on, packaged so it snaps together with any other part. Teams deposit the parts they build and draw on the parts every earlier team left behind. This is the LEGO logic of synthetic biology made literal: standard bricks with known behavior, open for anyone to reuse. It is also what turns a yearly competition into something cumulative. Each generation of projects adds to a catalog the next one inherits, so a team today begins where last year's teams stopped instead of starting again from raw DNA.
	
	The idea came from Tom Knight at MIT in the early 2000s, who wanted to engineer biology the way we engineer electronics \citep{endyFoundationsEngineeringBiology2005}. An engineer choosing a resistor from a catalog does not ask how it was made; she reads its rating and trusts it to behave. Knight wanted a genetic part to carry the same kind of guarantee, so that building a living circuit could become a matter of choosing components whose behavior was already documented. The standard that made this possible was published as a community Request for Comments, the same open document format the internet's protocols are written in, and it specified the flanking DNA that lets any two compatible parts be joined in a single reaction \citep{shettyEngineeringBioBrickVectors2008}. The format was itself an argument: a standard for engineering life should be published in the open and owned by its community, the way the protocols that run the internet are.
	
	The Registry of Standard Biological Parts opened with the first iGEM competition in 2004. Every team is expected to leave at least one characterized part behind, so the registry has grown in step with the competition, from a few dozen parts that first year to roughly 89,000 by the time we downloaded it. Each entry holds the DNA sequence, a functional description, a type, the depositing team, and often a pointer to the paper the sequence came from. Composite parts, the ones built by joining simpler parts together, list their components, so the catalog doubles as a map of how small parts combine into larger ones. Read a single composite entry and you can follow it down through its pieces to the original sequences it was assembled from.
	
	Assembly itself has moved on since Knight's first standard. The restriction-enzyme ligation it was built around is slow and prone to error, and for anything complicated it has given way to faster methods like Gibson assembly and Golden Gate cloning. The chemistry changed. The identifier did not. Teams still deposit in the standard format and still cite registry parts by their \texttt{BBa\_} numbers in their papers and project wikis, so a \texttt{BBa\_} code has become a stable handle for a piece of biological function, whatever technique actually stitched it into a given construct.
	
	This is what makes the registry useful to us as more than history. The parts a city's teams deposit are a physical record of what those students set out to build, and that record owes nothing to the words they wrote about the work. A team chasing biosensors leaves behind reporters and inducible promoters; a team engineering a metabolic pathway leaves composite parts that chain enzymes end to end. The registry knows nothing about embeddings or topic clusters. It records only the parts themselves, and that independence is exactly what lets it check our work: if the topics our model reads out of a city's project wikis match the parts that city's students actually deposited, the agreement cannot be an artifact of how a model reads an abstract. It has to come from the biology. The Mantel test in the results chapter is built on this logic.
	
	\subsection{Booze Bugs}
	...
	\textit{Booze Bugs: Sun to Alcohol}, one of many teams that have worked on creating a sustainable fuel source. Conventional biofuel is fermented from starch and sugar, which puts fuel crops in direct competition with food; the Uppsala students wanted to skip the crop entirely. They turned to \textit{Synechocystis}, a cyanobacterium, one of the blue-green algae that have been running photosynthesis for billions of years, and engineered it to convert sunlight and carbon dioxide straight into ethanol and butanol. A microbe that drinks sunlight, eats carbon dioxide, and exhales fuel is a pure statement of what synthetic biology can promise, and when these sorts of technology are successfully scaled up to industrial levels they can prove incredibly transformative.
	
	The impact of the research did not end with the competition. Two supervisors of the Uppsala team, Daniel Camsund and Thorsten Heidorn, continued in cyanobacterial research in Peter Lindblad's lab at the same university, and in 2014 the three published a paper on engineered promoters for cyanobacteria in the \textit{Journal of Biological Engineering} \citep{camsundLacIRepressedPromoters2014}. In this paper, they cited a collection of genetic parts from the iGEM registry by their \texttt{BBa\_} identifiers, including one deposited by a former teammate. The Uppsala team contributed one called BBa\_K273019, a copper-inducible promoter: a genetic switch that turns a downstream gene on in the presence of copper, tuned to work inside their cyanobacterium. Five years after it was deposited by undergraduates for a competition, the same sequence appears as a cited building block in the peer-reviewed literature. That is the kind of link this project was built to trace.
	...
	This is no longer an idea that lives only in the laboratory. Different lineages of research have pushed these organisms toward industrial use, and are worth distinguishing because they map closely onto clusters described in the data chapter
	
	The first runs directly through cyanobacteria. Algenol, founded in 2006 and based in Fort Lauderdale, Florida, engineered cyanobacteria to secrete ethanol directly into the growth medium, bypassing the harvest and extraction steps that make algal biofuels expensive, similar to the Uppsala students. Joule Unlimited, a startup founded in 2007 out of Cambridge, Massachusetts, used cyanobacterial fermentation to produce a biodiesel substitute with no biomass harvest required. Photanol, a spinout from the University of Amsterdam, followed the same logic with organic acids and alcohols as target products, taking an explicitly academic-to-commercial pathway in which the founding science came directly from published research on cyanobacterial metabolism. Each of these firms appears, at least in part, in the USPTO patent record: their inventors file in the United States to protect commercially valuable inventions, and those filings are part of the patent corpus in this study. The cities they cluster in, Cambridge, Fort Lauderdale, Amsterdam, and a handful of others, are consistent with the cities active in the clusters identified and explored in the results chapter.
	...
	Before measuring whether a city's student projects and academic papers are topically aligned, we should explore some possible mechanisms for this alignment. Three mechanisms are outlined, and distinguishing between them matters for interpretation of results
	
	The first is direct knowledge transfer. Faculty members who supervise iGEM teams also run research labs; in many cases the iGEM advisor is a PI whose publications are in the same database as the papers we compare against. If a mentor's research agenda sets the direction of their team's project, alignment follows mechanically from the advising relationship. This channel is person-level: the transfer runs through an individual who appears in both registers, predicting that project topics should resemble the specific research interests of the faculty advisor, that alignment should be strongest in cities with large, research-active universities where advisors have deep publication records, and that student projects should precede any new papers the advisor writes on that theme by a few years, since the project is usually early-stage relative to a published result.
	
	The second is a shared information environment. Perhaps a city's students and its academics converge on the same topics not because they influence each other directly, but because they respond to the same stimuli. Funding priorities, the same institutional research focus, visiting seminars, and local firms whose problems become projects and papers simultaneously can all influence research priorities. This is a common-cause mechanism. Neither group needs to observe the other; both are shaped by the same environment. Under this mechanism, alignment might show up simultaneously across artifact types, with no preferred direction, and the lead-lag curve should peak near zero. It should also survive the departure of any individual mentor: if the environment drives the alignment, losing one advisor does not break the city's profile.
	...
	These three mechanisms are not mutually exclusive, and the data cannot cleanly identify which one dominates. What they share, and what makes the project-paper alignment meaningful regardless of which mechanism produces it, is the implication of a substantive connection between student and academic synthetic biology at the city level. The direct-transfer and shared-environment mechanisms both describe how knowledge is organized locally; the selection mechanism describes how capable people are allocated to places. In all three cases, a city where iGEM teams and academic labs work on the same specific topics is a city whose synthetic-biology ecosystem is coherent in a way that is unlikely to be manufactured by data artifacts. The robustness analyses in the results chapter are designed to rule out data artifacts, while the mechanisms describe what remains once they do. We stand up our assertion, then attempt to knock it down with every tool at our disposal.
	
	\subsection{The geography of innovation}
	
	Economic geography turns relatedness into a research program with real predictive power. The principle of relatedness holds that regions diversify into industries and technologies related to their current strengths, and the pattern is regular enough to forecast: knowing what a place specializes in today predicts what it will move into tomorrow
	\citep{hidalgoProductSpaceConditions2007, hidalgoBuildingBlocksEconomic2009}. The
	same logic has been carried from products, to patents, to scientific fields, and to more complex, high-value activities that regions compete to attract \citep{ballandComplexEconomicActivities2020}. Relatedness is the through-line, explaining why capabilities cluster and spread to some places and not others, why a region's past shapes its future.
	
	So innovation is not geographically neutral; knowledge does not flow frictionlessly to wherever it would be most useful. Instead, it stays sticky, concentrated near the people and institutions that produced it. Knowledge spillovers, the informal spread of ideas from one worker or lab to the next, are strongly local, fading with distance
	\citep{breschiKnowledgeSpilloversLocal2001}. Part of the reason is that the most
	valuable knowledge is tacit, learned by doing and passed hand to hand, master to apprentice. A discovery made in one city is more likely to seed a related advance in the same city than a similar one across the world. Geography shapes where innovation goes next. Since this is such a well established effect, it might be more of an interesting result to show that iGEM research \textit{isn't} geographically influenced; that would show that the vision of a truly \textbf{International} Genetically Engineered Machine had actualized, that Synthetic Biology had transcended borders and become a global collective.
	
	But \textit{how} do capabilities and knowledge cluster spatially? The answer is grounded in economic literature; Duranton and Puga trace agglomeration to three micro-level mechanisms that operate at the scale of workers and firms rather than regions: sharing, matching, and learning \citep{durantonMicroFoundationsUrban2004}. Sharing of knowledge happens when firms that concentrate together can sustain more specialized suppliers than any one firm could support alone, reducing input costs for all. Matching is more efficient in thicker markets: a researcher searching for a collaborator with a very specific skill finds one much faster in a city of ten thousand researchers than in a town of fifty. And learning is faster when dense co-location brings people into incidental contact. The conversation at a conference, the seminar attended on a neighbour's recommendation, the former colleague now at a competing lab; these sorts of mechanisms compound when the people and firms in question work in related areas. A shared vocabulary makes learning cheaper, overlapping skill sets make matching more likely, and complementary inputs make sharing more valuable. These are the micro-foundations of what the relatedness literature documents at grander scales.
	
	Alfred Marshall identifies a complimentary observation from three sources of spatial concentration: shared input markets, pooled labor, and knowledge spillovers \citep{marshallPrinciplesEconomics1890}. These can be mapped onto iGEM; the shared input is the BioBrick registry, an open-source toolkit that all teams draw from and contribute to, a commons reducing the cost of working on the same subfield regardless of location. Pooled labor is the graduate student and postdoc population that circulates between student teams and academic labs, the same people who learn a method on an iGEM team and apply it in a PhD project. Knowledge spillovers are the face-to-face transmission of tacit knowledge, exactly what co-location in a city is supposed to enable. Hallway conversations transfer know-how that raw citation data cannot hope to capture \citep{breschiKnowledgeSpilloversLocal2001}. iGEM is unusual in that the relevant ``labor market'' is transient; students move through an institution for a fixed, short period, unlike most workers moving between firms. Much of the shared input is also openly licensed rather than commercially supplied, but the logic of agglomeration still applies: cities where multiple forms of synthetic-biology activity concentrate together are cities where the externalities compound. The Marshallian framing predicts exactly the kind of local topical specialization we set out to measure.
	
	Jane Jacobs added a different kind of agglomeration mechanism particularly relevant for a young field like synthetic biology \citep{jacobsEconomyCities1969}. Where Marshall's externalities come from \textit{specialization} within an industry, Jacobs' externalities come from \textit{diversity} across industries. Cities that mix many different activities generate cross-pollination between fields, sparking new combinations and innovations. Synthetic biology is itself a product of this kind of encounter, sitting at the intersection of molecular biology, computer science, and chemical engineering, which had themselves all emerged from different disciplines. Fields that are still defining their own boundaries benefit more from this melting pot of a diverse city than from the Marshallian depth of a highly specialized one, because they need to borrow tools and concepts from adjacent areas as fast as they can generates its own. The empirical prediction is that synthetic biology should cluster in large diverse cities rather than narrow specialist clusters, broadly consistent with the distribution of iGEM activity and academic output in this corpus.
	
	If capabilities are local and path-dependent, then measuring them matters, but measuring them well is difficult. Capability and innovation are abstract; we only ever observe the traces, outputs a research process leaves behind. To study them at scale we need those traces in a quantitative form, standardized enough to compare across places and times. The choice of what to measure, and how, carries a tremendous amount of weight here; measures that captures the wrong thing will describe the world confidently and incorrectly. We explore a range of measures, with a range of correctness levels, further in the results section. Most of the methodological care in this project has gone into getting these measures to reflect real relatedness rather than an artifact of how the data is being manipulated.
	
	The natural place to measure knowledge spillovers across firms and industries is the city. Innovation in deep-technology fields concentrates in cities; cities gather all the ingredients research needs into one place. Universities with equipment and students, firms with adjacent expertise, investors willing to fund risk, and a dense labor market that lets people and ideas move between them. Boschma and colleagues study exactly this at the city level, tracing how scientific relatedness shapes which biotechnology capabilities a city develops over time
	\citep{boschmaScientificKnowledgeDynamics2014}. The city is small enough to be a coherent unit and large enough to contain a full innovation ecosystem, making it the right unit of analysis for comparing research across projects, papers, and patents.
	
	Two further arguments support the city as the unit of analysis. The first is the geography of labor markets. Workers, students, and researchers circulate within cities far more than between them: a graduate student commutes between university and a collaborating lab; a postdoc moves between two institutions in the same metropolitan area; a founder recruits from the alumni pool of the nearby university. The city approximates the catchment area of this circulation, capturing the labor market in which capabilities pool and recombine \citep{durantonMicroFoundationsUrban2004}. A coarser unit, like a national region or a country, would pool cities with very different specializations into one observation, blurring the within-country heterogeneity that is most interesting (it's also worth noting that in an industry dominated by a few large US and Chinese cities, country-level comparisons are quite distorted). A finer unit, like a neighbourhood or a campus, would miss the cross-institution dynamics that depend on city-level density. The second argument is practical: the city is a standard geographic unit used worldwide at which all three of our artifact types can be geocoded with reasonable reliability. Patent inventor addresses resolve to cities, while author affiliations and iGEM teams resolve to the cities of their host institutions.
	
	\subsection{Measuring relatedness}
	
	To ask whether a city's projects, papers, and patents are related, we need a way to measure relatedness that works across all three, and the three could hardly be more different on the surface. A student wiki, a journal abstract, and a patent claim are written by different people, for different readers, under different rules, in different registers. A method that measures relatedness between two papers may say nothing useful about a paper and a patent. Before any of the geographic questions can be answered, we need a yardstick that reads across artifact types, and fortunately Economic geography offers several candidates.
	
	There are several methods to evaluate relatedness between industries or technologies, each with an array of strengths and weaknesses \citep{neffkeSkillRelatednessFirm2013a}. Citation networks are a simple method of determining co-occurence between artifacts such as papers or patents; if there is a citation linking two artifacts, they are related. Semantic overlap, measured as keyword co-occurence as used in \citep{boschmaScientificKnowledgeDynamics2014} is also widely used. Technological class co-occurence is another simple measure that can be used to link patent filings. Revealed Co-Occurence marks two entities as related if they show up together more often than chance would allow \citep{hidalgoProductSpaceConditions2007}. Relatedness can also be calculated by resource flows between two entities; for instance, if one firm buys a product from another, or if employees from one regularly seek work at another (Neffke Henning 2012). However, each of these measures has drawbacks. Citations can reflect other biases and are often a targeted measure, leading to manipulation \citep{bacciniGlobalExploratoryComparison2023}, while not being conducive to links between different artifact types. Keyword co-occurence loses some accuracy as fields and their associated vocabulary evolves over time, due to the concept of keyword drift. Technology classes only can measure relatedness between patents, and don't accurately measure emerging fields (like SynBio) \citep{jiangNaturalLanguageProcessing2025}. Revealed Co-Occurence requires spaces for artifacts to co-occur \textit{in}. We're aiming to measure relatedness across several different artifact text types in a fast-moving field, so we need a measure that's sector-agnostic. Keyword co-occurence would be principled and established in the literature, but keyword drift could get in the way of a meaningful measure that holds up across decades of time-series data.
	
	The measure this thesis arrives at is best understood against Boschma's taxonomy of proximity types \citep{boschmaProximityInnovationCritical2005}. Boschma argues that productive interaction between actors requires proximity along at least some of five dimensions: cognitive, organizational, social, geographical, and institutional, with cognitive proximity closest to what we are measuring. This is the degree to which actors share a knowledge base and can therefore absorb and evaluate each other's work. Two groups with high cognitive proximity, working on the same technical problems with the same methods, are more likely to benefit from knowledge exchange than two groups that happen to share a postcode but work in different areas of the field. Our semantic topic overlap is a proxy for cognitive proximity revealed through documented output. If two cities' student and academic communities are cognitively proximate to the extent that they concentrate on the same specific research clusters. Geographical proximity, which is what co-location in a city provides, is a necessary condition for the face-to-face interaction that transmits tacit knowledge, but it is not sufficient for knowledge exchange without the cognitive proximity to use what is shared. Both are present, by construction, in the cities where iGEM teams and academic labs co-locate around the same topics. Students and academics share both a knowledge domain and a geographic location, which the agglomeration literature identifies as productive \citep{breschiKnowledgeSpilloversLocal2001}.
	
	For this project, though, most of these measures do not fit, and understanding why clarifies exactly what we need.
	
	The input-based measures, those built from shared labor or skills, need data on who is moving where. Labor flows between iGEM alumni and biotech firms or shared personnel between student teams and patent applicants might be interesting to stuyd, but no such dataset exists at the scale or coverage this project needs. The few studies that have followed iGEM alumni into careers do so with small, hand-curated samples
	...
	The closest fit among the standard tools is textual. We must measure relatedness from the words the documents use, relying on the properties they have in common. Every project, paper, and patent has a title and an abstract, written in the vocabulary of the field, so keyword overlap is at least available across all three. Boschma and colleagues use exactly this kind of textual relatedness to study biotechnology at the city level \citep{boschmaScientificKnowledgeDynamics2014}. However, keyword matching is blunt, and misses that two documents can describe the same thing in different words. It stumbles precisely where synthetic biology is hardest, on a shifting vocabulary shared with molecular biology
	\citep{oldhamSyntheticBiologyMapping2018}.
	
	The probabilistic topic model is one step up from keyword overlap. Latent Dirichlet Allocation \citep{bleiLatentDirichletAllocation2003} represents each document as a mixture over a fixed number of latent topics, with each topic represented by a probability distribution over words. Documents about cyanobacteria and ethanol production land in similar topic mixtures even when they use different specific terms, because the underlying vocabulary distributions are closely related. This method is far more advanced than keyword counting, finding shared subject matter beneath surface word choice. It can also be applied to any document with text, including patents and student wikis. The problem is that the topics are fit separately for each document type or, if fit jointly, are dominated by the largest and most vocabulary-rich document type. In a corpus where papers outnumber projects and patents several times over, the latent topics are effectively paper topics discovered by a model that has seen far fewer patents and wikis. When a student project is mapped into those paper-derived topics, the assignment reflects how well the wiki's words fit the paper vocabulary, not how related the project is to the paper's subject. More fundamentally, topic models encode vocabulary distributions and nothing else; they cannot leverage the citation structure that tells us which documents humans have already judged to be related, and they cannot transfer knowledge from a large pre-trained corpus about the deeper semantic relationships that citation-trained models encode. For cross-type comparison in a field with heterogeneous writing styles, topic models solve the wrong problem.
	
	Citations are the other content-based option, and they are appealing because they represent explicit, human-made links between two documents. They are strong evidence of relatedness when they exist, and a long tradition uses citation networks to trace the structure of science and its links to technology \citep{narinIncreasingLinkage1997}. Unfortunately, citations are sparse across the boundaries we care about. Student projects rarely cite each other in any standard form, patents cite papers unevenly, and the cross-type links that do exist, like the BioBrick citations we recovered, are a thin and biased sample of the true connections. Citations also carry their own distortions, shaped by strategic and social pressures as much as by intellectual debt
	\citep{bacciniGlobalExploratoryComparison2023}. As a primary measure across three artifact types, they leave too much of the graph empty, although they do form an important part of our model training data. Instead, we will use a newer, but still well-established, tool for calculating relatedness: semantic embeddings.
	...
	The first practical versions of this were built through our old friend keyword co-occurence; as linguist JR Firth, put it "you shall know a word by the company it keeps"\citep{firthSynopsisLinguisticTheory1957}. Later research represented documents as vectors in a space with one axis per word, then measured how close two documents were by the angle between their vectors. In the 1990s, latent semantic analysis expanded on this, taking a big, sparse matrix of which words appeared in which documents and used linear algebra to compress it, squeezing thousands of word-axes down to a few hundred that captured the strongest patterns of co-occurrence \citep{deerwesterIndexingLatentSemantic1990}. 
	
	The modern version of these semantic embeddings originated from the labs of Google in the form of Word2vec \citep{mikolovEfficientEstimationWord2013}. They took a body of text, transformed it into a matrix with each word as a vector, and trained a small neural network to adjust the weights so that words which appeared next to each other would have corresponding nearby vectors. No single number in each vector meant anything you could name, but the geometry of the space captured how words relate to one another. The famous demonstration was taking the vector for "king," subtracting the vector for “man," adding “woman," and landing next to "queen" in the semantic space. Relationships between words thus become abstracted into the relationships between vectors, and meaning could be uncovered through empirical analysis.
	
	The following generations of NLP (natural language processing) models refined this strategy by making a word's vector depend on the sentence around it, so the same word gets different coordinates in different contexts. These contextual models are built on the transformer, an architecture introduced in 2017 \citep{vaswaniAttentionAllYou2017}, and the best-known early example is BERT \citep{devlinBERTPretrainingDeep2019}, which reads a sentence in both directions at once to work out what each word means. The same machinery was developed to embed sentences, then documents, into single vectors.
	
	SPECTER2, the model we use as a base for our semantic space, is a document embedder from the BERT family, built by the Allen Institute for AI \citep{singhSciRepEvalMultiFormat2023}. It's especially well suited to our purposes because of its specialized training; rather than guessing neighboring words, it was trained on scientific abstracts and citation data, learning to place two papers' vectors close together when a citation links them \citep{cohanDocumentlevelRepresentation2020}. It turns each abstract into a 768-dimensional vector, and we take the cosine distance between two vectors as a measure of relatedness between the underlying texts. We fine-tune SPECTER2 specifically to process patents, iGEM projects, and SynBio publications, so that we can calculate relatedness across disparate artifact types. This lets us drop a student iGEM project, an academic paper, and a patent into the same embedding space and answer a question: are these three things, produced in the same city under very different incentives, based in the same intellectual neighborhood? If they are, it could be inferred that there's some interaction taking place, that the students are meaningfully embedded in the research landscape of the city.
	
	\subsection{Three artifacts, One Space}
	...
	Answering our question meant building a corpus that did not yet exist. Student projects, academic papers, and patents in synthetic biology, each geocoded to a city, tied to a year, tied to text and embeddings. The three come from different worlds and arrive in different shapes, so much of the work was in the collecting and the cleaning.
	
	We pulled them into one shared schema, a common set of fields every artifact carries whatever its type. A unique identifier, an artifact type (project, paper, patent, or part), a title, the abstract or description text used for embedding, a year, a city name with geographic coordinates, the source country, and a case-study flag for the carbon-capture subset standardized each artifact and let them be compared. Additional metadata, like the parts a project built, the citation links recovered, and the fine-tuning weights of each training pair, live in supplementary files linked by identifier. The schema is a design choice that makes the downstream analysis as clean as possible. Once every artifact wears the same fields, the differences between a patent claim and a student wiki stop mattering for the parts of the analysis that follow. A distance computation, a cluster assignment, or a city-level overlap all look the same whether the artifact was filed in a patent office or written by an undergraduate.
	...
	\begin{figure}[htbp]
		\centering
		\includegraphics[width=\linewidth]{world_map.png}
		\caption{Geographic distribution of synthetic biology artifacts across cities. Each panel shows one artifact type: iGEM student projects (left), academic papers (centre), and patents (right). Circle size is proportional to the square root of artifact count in that city. Projects are the most geographically concentrated, reflecting the university-competition structure of iGEM; papers reach more cities because a broader range of institutions publish; patents are the sparsest, concentrated in established biotechnology hubs. Annotation in each panel reports the total number of geocoded cities and artifacts. The pattern confirms that the corpus spans multiple continents, though it is weighted toward North America and Europe by the structure of all three data sources.}
		\label{fig:worldmap}
	\end{figure}
	...
	\begin{figure}[htbp]
		\centering
		\includegraphics[width=\linewidth]{data_provenance.png}
		\caption{Data pipeline and artifact counts at each processing stage. Three independent streams --- iGEM Registry, OpenAlex, and the Oldham et al.\ synthetic biology patent corpus --- are geocoded independently, then linked via cross-type citation pairs for fine-tuning. The fine-tuned SPECTER2 model embeds all 17,826 artifacts into a shared 768-dimensional semantic space, which is then projected and clustered to yield 84 topic clusters. City-level Jaccard overlap is computed for the 387 cities where at least two artifact types are co-present. Geocoding attrition is below 1.5\% for each stream; the 21,959 project-to-paper wiki-DOI links provide the primary fine-tuning signal for cross-type alignment.}
	...
	Now that we've got the corpus built, it's time to dive in and see what we can find. This chapter questions the data directly, uses it to build up a hypothesis, and tries as hard as possible brutally break that assumption down. First, we propose a measure of whether a city's projects, papers, and patents are related, check what it says, and then attack it with robustness checks. We challenge the model's assumptions, search for sampling artifacts, and try to break down and defend the core identity of the project itself. We also run an independent validation from data that the measure never touched to corroborate our findings, and see what's survived the onslaught.
	
	The results chapter can be roughly organized into three arcs. The first is about measurement: our initial idea built up, then broken by a fundamental flaw. This leads into understanding on why it fails, leading directly into a more robust measure that stands up to the same tests. Then, having found a positive result, it's time for robustness checks. We vary everything that could have manufactured or biased our result, dropping countries, changing cluster counts, adjusting the city-depth threshold, and adding a regression and a difference-in-differences model. We put the result in a form that aims to invite comparison with the economic-geography literature. Finally, we check our pattern against an unrelated measure outside the semantic space entirely, based on the distributions of genetic code created by the iGEM projects. This final check is the one that removes the last legitimate worry about whether we have found something defensible or an artifact invented by our model and an untested assumption.
	
	\subsection{Project-Paper Basic Alignment}
	
	Before asking about city-level structure, we should ask the simplest possible question of the data. Is any given project semantically closer to the papers in its own city than to papers elsewhere? 
	
	For each of the \statAlignNProjects{} projects with a city assignment and at least one same-city paper, we computed two numbers: the average cosine similarity between the project and all papers from the same city, and the average cosine similarity between the project and papers from a random set of other cities of the same size. The difference between these averages is the project's local alignment. If the project sits closer to the vector of its own city's average research than to the rest of the field, it will be positive.
	
	The result is clear. The mean alignment across all \statAlignNProjects{} projects is $\bar{\delta} = \statAlignMeanDelta{}$, and \statAlignFracPos{} of projects show a positive alignment. A one-sample paired $t$-test on the distribution of individual alignment scores gives $t = \statAlignT{}$ (unclustered) and $t = \statAlignTClustered{}$ when errors are clustered by city to account for the fact that projects from the same city share the same local paper environment. Both values are far beyond any conventional significance threshold. At the city level, \statAlignFracCitiesPos{} of cities show a positive mean alignment across their own projects. The mean alignment for large cities (above the median paper count) is $\statAlignDeltaLarge{}$, versus $\statAlignDeltaSmall{}$ for small ones ($t = \statAlignTCity{}$ at the city level), significantly larger, which urged some followup question. Perhaps the larger cities are able to specialize more in their research due to greater ability to find human capital? Do larger cities have richer local paper environments that provide a more sharply defined semantic neighborhood, so their projects have more room to align locally rather than to the global average? Or perhaps there's something else.
	
	This is the most simple and direct test of the thesis's central claim, and it passes clearly. At the level of individual projects, the average project in our corpus sits semantically closer to its own city's research than to research from other cities, by a small but highly consistent margin. The finding sets the stage for the city-level analyses that follow.
	
	The first measure is a natural comparison between vector averages. For each city $c$, let $P_c$ be its set of projects and $Q_c$ its set of papers. The centroid of the projects is the average embedding:
	\begin{equation}
	...
	\end{equation}
	This ranges from $-1$ (perfect opposition in meaning) to $+1$ (perfect alignment), with $0$ denoting no consistent directional relationship. Cosine similarity is standard for high-dimensional embeddings because it ignores vector magnitude and focuses on orientation, so two documents that are both about cyanobacteria will point in roughly the same direction through the 768-dimensional space. It is also what SPECTER2 was trained to optimize for citation prediction, so the geometry the model learned is exactly what cosine similarity reads. Computed across all cities with enough documents in both corpora, the mean similarity is \statOverlapMean{} and the median is
	\statOverlapMedian{}, on a scale where 1 is identical. 
	
	Taken at face value, this looks like a resounding "Yes!" to our question! An immediately significant result! Almost every city's students and academics are pointing at the same part of conceptual space. 
	
	Alas, this was too good to be true. The centroids are high everywhere, including for city pairs that share no obvious subject, which signals that the measure is picking up something other than local relatedness. We tried our first robustness check, and we regressed the overlap on the log of each city's document count:
	\begin{equation}
		\text{sim}(\bar{\mathbf{p}}_c, \bar{\mathbf{q}}_c)
		= \alpha + \beta_1 \log|P_c| + \beta_2 \log|Q_c| + \epsilon_c
	\end{equation}
	The finding crumbles under a bit of pressure. Size dominates, overlap correlates at \statSizePapersR{} with paper count and
	\statSizeProjectsR{} with project count (Figure~\ref{fig:centroidsize}). The mechanism is
	a known property of high-dimensional averaging, sometimes called centroid collapse. Each document's embedding lives in a large space with a dense cluster near the origin where all of a field's general vocabulary concentrates. As a city accumulates more documents, its centroid is pulled steadily toward that dense centre, where the generic vocabulary of all of synthetic biology overlaps. Large cities end up with centroids near each other and near the global mean, not because their students and academics work on the same things, but because averaging across a large, mixed corpus washes away the specificity that would distinguish one city's focus from another's. The effect runs equally for cities with genuinely aligned projects and papers and for cities whose students and academics work in entirely different subfields. Everyone is equal in the eyes of this semantic vortex, and they all compress to centre as the corpus grows. The centroid measure significance is a size artifact. It answers a question about how much a city publishes, dressed up as a question about what it publishes. This was the first real result of the project, and it was a negative one, but the mechanism pointed directly at the fix so it turned out to be a blessing in disguise.
	
	\subsection{Better Alignment throught Clusters}
	...
	The failure of the centroid measure directs us towards its own fix. The problem was averaging; if you collapse a city's entire research output into one point, you lose the specificity of what it works on. The fix is to stop averaging and start measuring topics, so instead of representing a city as a single vector, we represent it as a profile across the 84 clusters from the previous chapter. For each cluster, how many of the city's documents fall into it? A city is now a distribution over topics rather than a point in space. Relatedness becomes a sharper question. It's no longer "do the city's students and academics point in roughly the same direction on average?" Instead, we measure if they specifically co-invest in the same narrow subjects. A city might be enormous and productive across every topic in the field, earning a high centroid similarity with everyone, while its students and academics work on entirely different subfields. The topic-profile approach distinguishes that case from a city where both groups genuinely concentrate on the same handful of clusters. That distinction is the one the thesis is built to make.
	...
	To check, we replaced the semantic HDBSCAN clusters with $k$-means at a range of resolutions, sweeping $k$ from fine to coarse, and recomputed the permutation excess for all three pairwise combinations — project$\times$paper, paper$\times$patent, and project$\times$patent — at each value (Figure~\ref{fig:kcurve}). Project$\times$paper alignment is the strongest and most consistent: the excess is positive and significant at every reasonable resolution. Paper$\times$patent alignment is also positive and significant across the sweep, though at a lower level. Project$\times$patent alignment is the weakest of the three: the excess barely rises above zero and the permutation p-values stay above 0.05 across the entire range of $k$. The hierarchy is stable across all resolutions. Projects relate most strongly to papers, papers relate to patents, but the direct project-to-patent link never achieves statistical significance. This is not a gap in the analysis; it is a substantive finding. It means the three artifact types are not equally related to one another, and that academic research functions as the middle link in the chain. A student project is connected to the commercial patent landscape through the papers it is co-located with, not directly.
	
	\begin{figure}[htbp]
		\centering
		\includegraphics[width=0.92\linewidth]{robust_kcurve.png}
		\caption{All three pairwise alignments across the full $k$-sweep (left: permutation excess over null; right: p-values). Project$\times$paper and paper$\times$patent both stay significant at every reasonable resolution. Project$\times$patent never crosses the significance threshold (dashed line, right panel), establishing the ladder structure in which academic papers mediate between student projects and patents.}
		\label{fig:kcurve}
	\end{figure}
	
	The $k$-sweep also settles the question of whether the fine-tuned embedding model inflates the result relative to the base SPECTER2. Table~\ref{tab:robustness_kcurve} runs the same permutation test at six values of $k$ for both models side by side. For paper$\times$project, both models produce significant excesses at every resolution, and the magnitudes are comparable, giving no indication that fine-tuning is manufacturing alignment that the base model does not see. For paper$\times$patent, the fine-tuned model shows somewhat larger excesses, which is expected: the training data included patent-paper pairs from the Marx matching, so the adapter was explicitly trained to bring those document types closer together, and it did. That the result is present in the base model as well confirms it is real; that it is amplified by fine-tuning is an expected consequence of what the fine-tuning was designed to do. For project$\times$patent, the base model shows no consistent significance across the six resolutions, and the fine-tuned model is mixed. Neither model produces a stable project-patent alignment. The conclusion the $k$-sweep supported — that academic papers are the middle link and the direct project-to-patent link is weak — holds in both models.
	
	\input{table_robustness_kcurve}
	...
	Table~\ref{tab:size_robust} runs the regression progressively, adding controls one at a time. Adding only $\log(\text{papers})$ in column~(2) gives $R^2 = 0.059$ and a coefficient of $0.020$ ($p = 0.019$). Column~(3) adds $\log(\text{projects})$, reaching $R^2 = 0.064$, the value reported above. Columns~(4) and~(5) add the duration controls: iGEM year span and the year of a city's first project, which together proxy for how long a city has had time to accumulate both corpora. These controls attenuate the log-papers coefficient to $0.016$ and widen its confidence interval ($p = 0.125$), which means roughly a quarter of the raw correlation reflects temporal depth rather than alignment per se. The residual is still positive, and the permutation test established it is real; the regression is saying that most of what drives it is not captured by any observable we have. That is the honest description of the data: alignment exists, it survives size and country controls, and it is largely unexplained in the cross-section.
	
	\input{tab_size_robust}
	
	Three further checks probe whether the alignment result is fragile. Table~\ref{tab:tercile} splits the 60-city sample into terciles by corpus size. None of the three splits produces a significant coefficient on $\log(\text{papers})$, which is expected: each tercile holds only 19 to 21 cities, far too few to resolve a signal of this size. Column~(5) of the same table adds $\log(\text{papers})^2$ to test whether the relationship peaks at intermediate depth and declines in the largest cities. The quadratic coefficient is positive but not significant ($p = 0.23$). There is no evidence that the depth-alignment relationship reverses sign at scale; if anything, the large-city tercile shows a numerically higher point estimate than the others. Table~\ref{tab:geo_robust} drops the United States and China in turn, the two countries that between them supply more than half the sample cities. In every geographic restriction the $\log(\text{papers})$ coefficient stays positive. It reaches significance when China alone is dropped (Drop CN: $b = 0.023$, $p = 0.022$) and in the non-OECD subsample ($b = 0.041$, $p = 0.026$). Dropping both US and China together reduces the sample to 26 cities and significance falls to $p = 0.106$, but the sign does not change. No single country group is driving the result.
	
	\input{tab_tercile}
	\input{tab_geo_robust}
	\input{tab_cc_moderator}
	
	A natural question is whether the result is specific to carbon capture: if cities doing more carbon-capture work show systematically higher or lower alignment, the finding would describe that subfield rather than synthetic biology at large. Table~\ref{tab:cc_moderator} tests this by interacting $\log(\text{papers})$ with the share of a city's clustered documents in the carbon-capture topic clusters, both for papers (columns~1--2) and for projects (columns~3--4). Neither interaction is significant. The carbon-capture paper interaction coefficient is $0.49$ ($p = 0.245$); the project interaction is $-0.03$ ($p = 0.925$). Column~(5) enters both interactions jointly; neither moves. The main effect of $\log(\text{papers})$ remains positive and numerically stable across all five specifications. Carbon-capture cities are not special in this respect. The depth-alignment relationship is a general feature of where synthetic biology is practiced, not a signal confined to the one subfield where we followed the case study.
	
	The cross-sectional regression shows that alignment exists; it cannot say whether it is a living year-to-year coupling or a structural feature baked into each city's long-run specialization. A difference-in-differences design separates the two. We split the sample into five-year periods (2004--2008, 2009--2013, 2014--2018, 2019--2024) and ask, within a city and topic, whether rising activity in an upstream artifact type in one period is accompanied by rising activity in the downstream type. For each of the three ordered pairs---project $\to$ paper, paper $\to$ patent, and project $\to$ patent---let $Y_{c,k,t}$ be the downstream type's share of cluster $k$ in city $c$ in period $t$, and $X_{c,k,t}$ the upstream share. We fit:
	\begin{equation}
		Y_{c,k,t} = \alpha_c + \delta_{k,t} + \beta\, X_{c,k,t} + \epsilon_{c,k,t}
	\end{equation}
	where $\alpha_c$ is a city fixed effect, absorbing each city's standing topic profile across all periods; $\delta_{k,t}$ is a topic-by-period effect, absorbing how popular cluster $k$ is in period $t$ across all cities; and $\beta$ captures the residual within-city, within-topic co-movement that remains after both are differenced out. Identification rests on the Frisch-Waugh theorem: we partial out the fixed effects by within-group demeaning rather than including dummy variables, which is equivalent but computationally cleaner in a three-way panel. Standard errors are clustered by city throughout.
	
	Figure~\ref{fig:did} shows the three estimated $\hat\beta$ coefficients with 95\% confidence intervals. The sample for this exercise is restricted to cities present in the tripartite corpus, those with enough documents of all three types to estimate cluster shares, which yields 44--46 cities per pair, compared to 161 cities in the bipartite project-paper analysis. Within that smaller sample, the paper $\to$ patent link is positive and significant ($\hat\beta = +0.355$, $p = 0.028$, $N = 44$ cities): within a city and topic, a five-year period of elevated academic output predicts elevated patenting in the same topic. The project $\to$ paper link has a point estimate that is negative and not significant ($\hat\beta = -0.085$, $p = 0.19$, $N = 46$ cities), which contrasts with the positive and significant bipartite result ($\hat\beta = +0.0026$, $t = 4.06$, $N = 161$). The discrepancy most likely reflects power: the tripartite sample is less than a third the size. The project $\to$ patent link, estimated on only ten cities, is not interpretable. The paper $\to$ patent finding, grounded in 44 cities, is the new result from this extension: within a city, academic output in a topic predicts commercial filings in the same topic, in a way that is not accounted for by either the city's historical specialization or the topic's global commercial trend.
	
	\begin{figure}[htbp]
		\centering
		\includegraphics[width=\linewidth]{tripartite_did.png}
		\caption{Tripartite difference-in-differences: estimated $\hat\beta$ coefficients with 95\% city-clustered confidence intervals for all three ordered pairs. A positive coefficient means that within a city and topic, the upstream artifact type's share in a five-year period predicts a higher downstream share in the same period, after city baselines and topic trends are differenced out. Paper $\to$ patent is positive and significant ($\hat\beta = +0.355$, $p = 0.028$, $N = 44$ cities). Project $\to$ paper is not significant in this smaller sample ($p = 0.19$, $N = 46$ cities), consistent with insufficient power relative to the 161-city bipartite analysis. Project $\to$ patent uses only ten cities and is not interpretable.}
		\label{fig:did}
	\end{figure}
	
	\subsection{Lead-lag structure}
	
	The difference-in-differences result found a small dynamic coupling within five-year windows. A natural follow-up is whether that coupling has a preferred direction across years: do student projects tend to precede academic papers in the same subfield, and do papers precede patents? To probe this, we built annual cluster-frequency vectors per city per artifact type: for each city-year with at least two non-noise documents of that type, we computed an $L_2$-normalised $K$-dimensional histogram over the 80 fine-tuned topic clusters. For each lag $\ell \in \{-3, \ldots, +3\}$ years, we then computed the average cosine similarity between a city's upstream vector at year $t$ and its downstream vector at year $t + \ell$. A positive $\ell$ means the downstream artifact comes after the upstream one. Bootstrap 95\% confidence intervals were formed by resampling city-year pairs 500 times.
	
	Figure~\ref{fig:leadlagprofile} shows the lead-lag profile for both pairs. Both profiles are relatively flat across lags, with confidence intervals that overlap substantially at all values of $\ell$. There is no visually clear asymmetry that distinguishes positive from negative lags. This is consistent with the small annual sample sizes: the tripartite corpus yields 104--161 city-year pairs per lag depending on the pair and the lag value, which is not enough to resolve the modest temporal signal that appeared in the larger bipartite analysis. The profiles are informative as a descriptive summary of how annual cluster similarity varies with the temporal offset, but they do not by themselves establish a directional ordering.
	
	\begin{figure}[htbp]
		\centering
		\includegraphics[width=\linewidth]{tripartite_lead_lag.png}
		\caption{Lead-lag profiles for the two adjacent pairs in the innovation ladder, with 95\% bootstrap confidence intervals. Top: project activity at year $t$ vs.\ paper activity at year $t+k$; positive $k$ means papers come after projects. Bottom: paper activity at year $t$ vs.\ patent activity at year $t+k$. Profiles are relatively flat across lags; the permutation tests (Figure~\ref{fig:leadlagperm}) do not find a statistically significant directional asymmetry at this sample size.}
		\label{fig:leadlagprofile}
	\end{figure}
	
	To test whether the observed profile shapes reflect real temporal structure or are consistent with chance, we ran a permutation test on both lead-lag shapes (Figure~\ref{fig:leadlagperm}). Within each city, we shuffled the year labels on the upstream type's annual vectors 500 times, recomputing the full profile at each shuffle. Two test statistics were computed per pair: the slope of a linear fit to the mean similarity as a function of lag, and the contrast $\text{mean}(\ell > 0) - \text{mean}(\ell < 0)$. In both pairs, the observed slope and contrast statistics fall well within the null distribution produced by year-shuffled cities; $p$-values range from 0.51 to 0.93. The tripartite lead-lag analysis does not find statistically significant temporal structure. This is a null result for this particular dataset and sample size, not a demonstration that temporal ordering is absent; the bipartite analysis on 161 cities produced a significant lead-lag asymmetry at the same lags, and the discrepancy is most naturally explained by the smaller and more selective sample available when all three artifact types must be present simultaneously.
	
	\begin{figure}[htbp]
		\centering
		\includegraphics[width=\linewidth]{tripartite_lead_lag_perm.png}
		\caption{Permutation tests of lead-lag asymmetry (500 within-city year shuffles), for both pairs. Each row is one pair; each column is one test statistic. Left column: the slope of the lead-lag profile against the null distribution. Right column: the contrast mean($\ell>0$) $-$ mean($\ell<0$) against its null. In both pairs the observed statistics fall inside the null distribution, indicating that the tripartite corpus does not produce a statistically significant temporal asymmetry at this sample size.}
		\label{fig:leadlagperm}
	\end{figure}
	
	The negative result on temporal ordering is not an argument against the main finding. The permutation test on the static, full-corpus cluster overlap---the core result of this thesis---operates on the same tripartite data and produces a highly significant positive signal ($p \approx 0.0005$). That test is powered by the full within-city co-membership distribution, not by year-by-year fluctuations. The DiD and lead-lag extensions ask a harder question with a smaller sample. The paper $\to$ patent DiD reaches significance, which is the new dynamic finding from the tripartite extension. The temporal ordering of project activity relative to paper activity, which was significant in the bipartite analysis, requires more data than the tripartite intersection currently provides to test directly in this framework.
	...
	The central claim is specific and modest. A city's iGEM projects and its academic papers share specific research topics more than size and topic popularity can explain, at a level of statistical confidence that a stringent permutation test places far beyond chance. That excess is not an artifact of one country, one cluster resolution, or one depth threshold. It is not a property of the embedding model, since it appears in the structure of the physical parts independently. And it is not merely a standing historical coincidence, since the difference-in-differences result finds a small but real year-to-year co-movement after each city's specialization is differenced out. The claim is comfortably defended as a real, size-independent local alignment between student and academic synthetic biology.
	
	Along the way the data drew a portrait of iGEM and what makes it special. It is a worldwide, open, standardized on-ramp into synthetic biology. It also leaves an unusually complete record, with thousands of student projects, each built from a shared kit of parts that connect back to the past literature and forward into future work. The Uppsala team turned sunlight into alcohol with blue-green algae, and two of its members carried that work into a research lab and a published paper that cited the team's own BioBrick. That thread, from a competition project to peer-reviewed science in the same city, is the whole thesis in miniature, and iGEM produces threads like it all over the world.
	
	The central finding is specific, with a shape worth stating plainly. In a given city, the three artifact types are not equally related to one another. A city's patents are related to its papers, and its papers are related to its student projects, but its patents and its projects are the most distant of the three pairs. The chain is strongest between neighbours, project to paper and paper to patent, and weakest across the full span from student bench to commercial filing. That is what one would expect if ideas and capabilities travel in steps, with academic research as the middle link between student work and industry, though our data shows only the pattern, and does not defend any causal assertions.
	
	Read together, the results support the narrative of student projects embedded in the local research scene. Where a city's academics work, its students tend to work too. More than chance or size can explain, students focus on the same topics as their geographical peers. The pattern is anchored in the physical parts the students build as much as in the words they write, a point deserving emphasis. The BioBrick class distribution is a record of students' actual choices about what to build, made in a different register from any text the language model could have read. The fact that it agrees with the semantic topic structure confirms that the relatedness is about research orientation, not about a shared vocabulary that a text model might have imposed. The Mantel result is moderate in magnitude precisely because parts and text are two different views of the same underlying thing, not the same view described twice.
	
	The difference-in-differences result adds a dynamic dimension to this picture. Most of the alignment is structural, a city's persistent concentration in particular subfields appearing simultaneously in its student and academic output, in the way that a city long invested in cyanobacterial research will produce both iGEM projects on cyanobacteria and publications on cyanobacteria. It doesn't necessarily mean one causes the other, just that both draw on the same local ecosystem. However, there is also a small year-to-year component, where when a city's student projects lean into a topic in one period, its academic output in that topic rises slightly beyond its baseline, and vice versa.
	
	It bears repeating, because it is easy to forget in the momentum of a clean result, none of this is a causal claim. We have not shown that student projects cause later papers, or that academic work seeds student projects, or that either drives a patent. The three artifact types share many local causes, with the same universities hosting student teams and research labs, the same mentors advise students and run labs, the same regional firms funding research and recruiting from the same talent pool. Indeed, the same funding agencies set priorities that shape what both students and academics choose to work on. Any or all of these common causes could produce the pattern we observe, with no direct flow of ideas from one artifact type to another required. We find relatedness and co-location, but we cannot, from this data, separate that from a common environment that produces both. The data does what observational data does best, documenting the pattern with enough rigor to rule out size and model artifacts as explanations. They do not rule out shared causes, and they are not designed to. Causal identification is a harder and different problem, requiring either a natural experiment or panel data on individuals, and it is one of the directions a follow-up should take.
	
	That limit is also an invitation. The relatedness we document is exactly the pattern a causal story would leave behind, creating a solid foundation for work designed to test one. Tracing named individuals from iGEM rosters into later authorships and patents, or following specific parts from the registry into commercial filings, could turn co-location into something closer to a trajectory. This thesis maps the ground of causal inference in this space, but does not yet attempt to traverse it. There are several such paths, left unwalked for want of time. A master's thesis has a fixed horizon, and a number of the richest questions sit just past it.
	...
	\section{Custom Claude Code skills}
	\label{app:skills}
	The reasoning aids described in the statement on AI use were built as custom Claude Code skills: plain-text markdown files that set each agent's role, scope, and voice. Two of them shaped the reasoning behind this thesis. The \texttt{frankGPT} skill cast the model as an economic-geography advisor in the mold of Frank Neffke, to pressure-test the framing and the empirical strategy; it is available in full in the project repository. The \texttt{discussion} skill staged debates among named researchers, and rather than describe it, this appendix demonstrates it. The prompt is reproduced below, followed by its output when the skill was turned on the question of whether these prompt files belong in a thesis appendix at all. Disclosing the prompt matters because tools like these shape the work as much as they speed it: recent science-of-science evidence finds that AI tools widen a researcher's reach while narrowing the questions they pursue \citep{haoArtificialIntelligenceTools2026}. Including that debate here is the choice the debate itself argues for: a prompt that shapes an argument is a researcher degree of freedom, and the honest way to handle it is to show it.
	% [FLAG: author to decide whether to also reproduce the full frankGPT skill file, or leave it as a repository pointer as done here.]
	
	\subsection*{The \texttt{discussion} prompt}
	
	\begin{lstlisting}[style=promptfile]
		You are running a structured academic discussion between three researchers about the topic or question the user provides. If the user provides no topic, ask what they would like to discuss.
		
		Simulate a back-and-forth dialogue with at least 3 rounds of exchange. Each researcher speaks in turn, reacts to what the others say, challenges weak claims, and builds toward a synthesis. Do not let any one voice dominate. End with a short synthesis paragraph that pulls together the key insights.
		
		---
		
		## The Researchers
		
		**Frank Neffke** — Economic geographer. Thinks in terms of agglomeration, relatedness, labor mobility, and causal identification. Skeptical of hand-wavy mechanisms; wants to know if the effect holds in a regression. Drily humorous, occasionally sarcastic.
		
		**Claudia Doblinger** — Innovation studies researcher focused on emergent technologies and clean-tech industries. Optimistic, motivating, practical. Grounds the discussion in real policy and industry dynamics. Agreeable but pushes back when theory drifts too far from the data.
		
		**Marc Santolini** — Network scientist and biophysicist embedded in the iGEM organization, studying the Science of Science. Excitable, intense, loves to zoom out to systems-level patterns. Brings in network topology, citation graphs, and open science dynamics. Sees connections others miss.
		
		---
		
		## Format
		
		Structure the output as a flowing dialogue, like this:
		
		**Frank:** [speaks]
		
		**Claudia:** [responds, agrees or pushes back]
		
		**Marc:** [adds a new angle or data point]
		
		**Frank:** [sharpens the argument or raises a counterpoint]
		
		...continue for at least 3 full rounds...
		
		**Synthesis:** [2-3 sentences pulling together the main takeaways]
		
		---
		
		The topic to discuss is: $ARGUMENTS
	\end{lstlisting}
	
	\subsection*{The \texttt{discussion} skill applied to its own inclusion}
	
	Run on the question of whether these prompt files should be reproduced in the appendix, the skill produced the following exchange. It was generated by the same tool it discusses.
	
	\textbf{Frank:} Let me start by being difficult, since that is what I am for. A regression specification is a method. A geocoding rule is a method. A prompt that tells a language model to ``argue like an economic geographer'' is not obviously a method; it might be set dressing. So before we decide to bind these files into the back of a thesis, I want an answer to a plain question: did the prompt change a number, or did it only change the prose around the number? If it only flavored the writing, it belongs in a footnote, not an appendix.
	
	\textbf{Claudia:} That is too clean a line, Frank. In practice the framing changes the analysis, not just the prose. If a prompt pushed the author to test a lead-lag structure rather than a static cross-section, it shaped what got estimated. And there is a norm question you are skipping. Journals have moved fast here: Nature, Science, and the major publishers now require authors to disclose how they used large language models and forbid listing them as authors. A thesis that used AI agents to help design and draft its analysis and then said nothing about how would look evasive in 2026, not rigorous. Reproducing the prompts is the honest version of a methods disclosure.
	
	\textbf{Marc:} And it is more interesting than mere compliance. Think about what these files are. They are the experimental apparatus. In the Science of Science we keep learning that the instrument shapes the finding, and here the instrument is a set of instructions that configured an agent's whole reasoning posture. That is exactly the kind of thing open science says you publish. We share code, we share seeds, we share the notebook. Why would the prompt that steered the reasoning be the one artifact we keep in a drawer? The whole iGEM ethos this thesis studies is that you deposit your part in the registry so the next person can build on it. The prompt files are this author's parts.
	
	\textbf{Frank:} Careful with that analogy, because it cuts both ways. A BioBrick is reproducible: the sequence is the sequence. An LLM prompt is not. Run it twice against two model versions and you get two different debates. So if the justification for the appendix is reproducibility, it is a weak justification. You are not letting me rerun the study. You are handing me a script for a play whose actors have already been recast.
	
	\textbf{Claudia:} But reproducibility was never only bit-for-bit rerunning, even in econometrics. Half of what a replication package buys you is the ability to see the decisions. The prompt is the documented decision. I cannot reproduce your fieldwork interviews either, Frank, yet you would still print your interview guide in the appendix so I can see what you asked and what you led the respondent toward. That is the right frame here. The prompt is a questionnaire, and you disclose the questionnaire.
	
	\textbf{Marc:} Yes, and the leading-question worry is the whole point in favor of printing them. Look at the frankGPT file. It literally instructs the model to prefer mechanism over description, to push toward relatedness and agglomeration framings, to be skeptical of a result until it survives a regression. That is a thumb on the scale, deliberately placed. A reader evaluating this thesis deserves to see that thumb. Hiding it would be the offense; showing it is a gift to the critic.
	
	\textbf{Frank:} That reframing I will actually accept. Not ``here is how to reproduce me,'' but ``here is the bias I introduced on purpose, read it and adjust.'' A prompt that shapes the argument is a researcher degree of freedom, and the cleanest way to handle a degree of freedom is to declare it out loud. On those grounds, print the ones that steered reasoning. But only those. If the author includes a prompt that reformatted a CSV, I will find them at the defense.
	
	\textbf{Claudia:} Agreed, and that is the discipline the appendix needs: curate, do not dump. Two or three files that changed how the work was reasoned about, each with a line on what it was for. Not the hundred utility instructions. A reader's attention is a budget, and spending it on scaffolding is its own kind of dishonesty because it buries the two files that matter under noise.
	
	\textbf{Marc:} And the last argument is the reflexive one, which I love too much to leave out. This thesis has a claim baked into it, that a human supplies the vision and the machine supplies the scale, and that the division of labor is honest and worth naming. If that claim is real, the prompts are the vision written down. They are the most direct evidence the thesis has that a person was steering. Omitting them would quietly contradict the thesis's own argument about how it was made. Including them makes the method match the message. The document about humans directing machine tools should show the directions.
	
	\textbf{Frank:} So we have talked ourselves from ``probably theater'' to ``load-bearing,'' which is more than I expected when I opened my mouth. Fine. Include them, frame them as declared instruments and declared bias rather than as a reproducibility guarantee, keep it to the files that moved the reasoning, and annotate each one. And put this very debate next to them, since apparently the instrument can be turned on the question of its own inclusion, which is either rigorous or unbearably clever.
	
	\textbf{Synthesis:} The three converge on a qualified yes: reproduce the reasoning-shaping prompt files (\texttt{discussion}, \texttt{frankGPT}) in the appendix, but frame them correctly. Their value is not bit-for-bit reproducibility, which an LLM cannot offer, but disclosure of a deliberately introduced framing bias, the same reason one prints an interview guide or a survey instrument. Include only the files that changed how the work was reasoned about, annotate each with its purpose, and let the choice double as evidence for the thesis's own man-and-machine claim, that the human hand steering the tools should be visible rather than hidden.
	
	\section{OpenAlex keyword query}
	\label{app:query}
	The paper corpus was built with a keyword query tuned against the vocabulary of synthetic biology, kept here for inspection and reproduction.
	% [FLAG: author to paste the exact OpenAlex query string used in the ingestion scripts]
