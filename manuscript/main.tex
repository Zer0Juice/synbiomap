% ============================================================
%  Patents, Papers, Parts, and Planet
%  Manuscript skeleton.
%  Text source: Write-Up.md (human-written rough draft).
%  Formatting follows the supplied thesis specification:
%    - Margins 2 cm on every side
%    - Times New Roman, 12 pt body
%    - 1.5 line spacing
%    - Justified text (Blocksatz)
%    - Directories (table of contents) in Roman numerals,
%      main text + bibliography + appendix in Arabic numerals
%    - Footnotes: 10 pt, single-spaced within, 1.5 spacing between,
%      hanging indent
...
% natbib gives author-year citations that read naturally in prose:
%   \citet{key} -> "Boschma et al. (2014)"  (author is part of the sentence)
%   \citep{key} -> "(Boschma et al., 2014)" (whole citation in parentheses)
% The bibliography itself (references.bib) is generated from the researcher's
% Zotero library by scripts/export_zotero_bib.py, so the keys here match the
% Better BibTeX keys shown in Zotero. Compile with:
...
% ============================================================
%  Footnote formatting
%  - 10 pt is automatic: in a 12 pt document \footnotesize = 10 pt.
%  - \singlespacing gives line spacing 1 *within* each footnote.
%  - \footnotesep gives the 1.5 line gap *between* footnotes.
%  - \@makefntext is redefined for a hanging indent (wrapped lines
%    align under the footnote text, not under the number).
% ============================================================
\makeatletter
% Single line spacing inside each footnote
...
\renewcommand\@makefntext[1]{%
  ...
  \makebox[1.8em][l]{\@makefnmark}#1}
\makeatother
% Gap between consecutive footnotes (~1.5x the 12 pt footnote leading)
...
% ============================================================
\section{Fermentation Technology}
% ============================================================
...
\citet{boschmaScientificKnowledgeDynamics2014} studied the impact of scientific relatedness on knowledge dynamics in biotechnology at the city level. This analysis provides a clean methodological base to build from.

\citet{santoliniIGEMModelSystem2023} presents the iGEM data as a well-structured dataset for studying team dynamics and innovation.
...
The research of \citet{oldhamSyntheticBiologyMapping2018}
...
\subsection{iGEM Projects data}

built from \url{https://teams.igem.org/}
...
SPECTER2 is an artificial intelligence model from the BERT \citep{devlinBERTPretrainingDeep2019} family of 2nd generation NLP (Natural Language Processing) models. As explained earlier, the document embeddings generated with it form the basis for much of this project's analyses.
...
% References
% ============================================================
% Pulled from the researcher's Zotero library via
% scripts/export_zotero_bib.py, which writes references.bib.
% \bibliography below emits its own unnumbered "References" heading;
% \addcontentsline puts that heading into the table of contents.
% \nocite{*} forces every entry in references.bib into the bibliography,
% including curated sources not yet cited in this draft. Remove this line
% once every reference is cited in the text above.
