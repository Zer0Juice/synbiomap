"""
Step 2 — Ingest patents from PatentsView (USPTO) using a layered keyword strategy.

PatentsView provides free, open access to all granted US patents via a REST
API. No API key or authentication is required.

Coverage note:
  PatentsView covers granted US patents only. It does not include European,
  PCT (international), or other national patents. The US is historically the
  largest single source of synthetic biology patents (van Doren et al., 2013),
  making this a reasonable primary source for this study.
...
with two keyword groups. CPC codes can be added as an optional AND filter
for higher precision (see config/settings.yaml: patent_cpc_codes).
...
Both layers are deduplicated on patent number. Each patent carries a
...
No credentials required — PatentsView is a free, open API.
"""
...
def _load_cache(layer_name: str) -> list[dict] | None:
    ...
    The cache stores dicts in the same shape as patentsview.extract_fields() output:
    {patent_id, title, abstract, year, city, country, lat, lon, retrieval_reason}
    """
...
def run():
    ...
    cpc_codes    = corpus_cfg.get("patent_cpc_codes")
    ...
    print("=== Step 2: Ingest Patents from PatentsView (USPTO) ===\n")
    print(f"Core keywords:     {core_kws}")
    print(f"Subfield keywords: {subfield_kws}")
    print(f"CPC filter:        {cpc_codes or 'none (keyword-only)'}")
    print(f"Max results/layer: {max_results}")
    print(f"Cache dir:         {CACHE_DIR}\n")
    print("No credentials required — PatentsView is a free, open API.\n")
